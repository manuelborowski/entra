    ...
    try:
        ...
        for wisa_item in wisa_data:
            ...
            if wisa_item['code'] in already_processed:
                continue
            wisa_item['geboortedatum'] = datetime.datetime.strptime(wisa_item['geboortedatum'].split(' ')[0],
                                                                    '%Y-%m-%d').date()
            email = wisa_item['email'] if 'campussintursula.be' in wisa_item['email'] else wisa_item[
                'prive_email'] if 'campussintursula.be' in wisa_item['prive_email'] else ""
            ...
            if email != "":
                ...
                wisa_item[
                    'email'] = f"{wisa_item['voornaam'].translate(normalize_letters).lower()}.{wisa_item['naam'].translate(normalize_letters).lower()}@campussintursula.be"
            ...
            if wisa_item['code'] in staff_in_db:
                ...
                if changed_properties:
                    changed_properties.extend(
                        ['delete', 'new'])  # staff-member already present, but has changed properties
                    wisa_item.update({'changed': changed_properties, 'staff': staff, 'delete': False, 'new': False})
                    changed_list.append(wisa_item)
                else:
                    flag_list.append({'changed': '', 'delete': False, 'new': False,
                                      'staff': staff})  # staff already present, no change
        ...
        log.info(
            f'{sys._getframe().f_code.co_name}, processed {nbr_processed}, new {len(new_list)}, updated {len(changed_list)}, deleted {nbr_deleted}')
    except Exception as e:
        log.error(f'{sys._getframe().f_code.co_name}, {e}')
